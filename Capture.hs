...
vars (Ref v) ss s1 s2     |(v `S.member` s1) = let v' = replace v s1 s2
                                                  in ((Ref v'), ((v, v'):ss))
                          |otherwise =  ((Ref v), ss)
vars (Abs v e) ss s1 s2   |(v `S.member` s1) = let v' = replace v s1 s2;
                                                   (e1', ss') = vars e ss s1 s2
                                                  in (Abs v' e1', (v, v'):(ss'++ss))
                          |otherwise = let (e',ss') = vars e ss s1 s2
                                              in (Abs v e', ss')
vars (App e1 e2) ss s1 s2 = let (e1',ss') = vars e1 ss s1 s2;
                                (e2',ss'') = vars e2 ss s1 s2
                                 in ((App e1' e2'), ss'++ss'')
...
list :: S.Set Var -> [Var]
list s = map (\v -> [v]) (filter (\v -> S.notMember [v] s) ['a'..'z'])
